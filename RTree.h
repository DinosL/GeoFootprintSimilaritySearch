...
public:
  ...
  bool Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const;
  bool Batch_Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region* queryRegions, int queryRegionsSize) const;
  vector<int> Batch_Search2(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region* queryRegions, int queryRegionsSize) const;
...
typedef pair <float, Node*> Contribution;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount) const;
  void internalLoopQ(Region rec, Node *pivot) const;
  void spatialSimilarity(int userRegionsSizeR, Region *userRegionsR, Node *userRegionsS) const;
  bool Batch_Search(Node* a_node, Rect* a_rect, Region *queryRegions, int queryRegionsSize, int& a_foundCount) const;
  bool Batch_Search2(Node* a_node, Rect* a_rect, Region *queryRegions, int queryRegionsSize, int& a_foundCount, vector<int>& hits) const;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
RTREE_TEMPLATE
vector<int> RTREE_QUAL::Batch_Search2(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region* queryRegions, int queryRegionsSize) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  vector<int> hits;
  Batch_Search2(m_root, &rect, queryRegions, queryRegionsSize, foundCount, hits);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Batch_Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region* queryRegions, int queryRegionsSize) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Batch_Search(m_root, &rect, queryRegions, queryRegionsSize, foundCount);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  // cout << "Level: " << a_node->m_level << endl;
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        float minX = max(a_rect->m_min[0],a_node->m_branch[index].m_rect.m_min[0]);
        float minY = max(a_rect->m_min[1],a_node->m_branch[index].m_rect.m_min[1]);
        float maxX = min(a_rect->m_max[0],a_node->m_branch[index].m_rect.m_max[0]);
        float maxY = min(a_rect->m_max[1],a_node->m_branch[index].m_rect.m_max[1]);

        float area = (maxX - minX)*(maxY - minY);	
        similarityPerUser[id] += area;          
      }
    }
  }
...
RTREE_TEMPLATE
void RTREE_QUAL::internalLoopQ(Region rec, Node *pivot) const
{
	int counter = 0;
	while(counter < pivot->m_count)
	{
    float area;
		if(rec.yStart > pivot->m_branch[counter].m_rect.m_max[1] || rec.yEnd < pivot->m_branch[counter].m_rect.m_min[1] || rec.xStart > pivot->m_branch[counter].m_rect.m_max[0] || rec.xEnd < pivot->m_branch[counter].m_rect.m_min[0])
		{
			counter++;
			continue;
		}
		float minX = max(rec.xStart,pivot->m_branch[counter].m_rect.m_min[0]);
		float minY = max(rec.yStart,pivot->m_branch[counter].m_rect.m_min[1]);
		float maxX = min(rec.xEnd,pivot->m_branch[counter].m_rect.m_max[0]);
		float maxY = min(rec.yEnd,pivot->m_branch[counter].m_rect.m_max[1]);
		area = (maxX - minX)*(maxY - minY);
    similarityPerUser[pivot->m_branch[counter].m_data] += area;	
		counter++;	
...
RTREE_TEMPLATE
void RTREE_QUAL::spatialSimilarity(int userRegionsSizeR, Region *userRegionsR, Node *userRegionsS) const
{
	int i=0,j=0;
	...
	{
			internalLoopQ(userRegionsR[currentR],userRegionsS);
			currentR++;
	}
...
RTREE_TEMPLATE
bool RTREE_QUAL::Batch_Search(Node* a_node, Rect* a_rect, Region *queryRegions, int queryRegionsSize, int& a_foundCount) const
{
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        if(!Batch_Search(a_node->m_branch[index].m_child, a_rect, queryRegions, queryRegionsSize, a_foundCount))
        {
          // The callback indicated to stop searching
          return false;
        }
  ...
  {
    spatialSimilarity(queryRegionsSize, queryRegions, a_node);
  }
  return true; // Continue searching
}
...
RTREE_TEMPLATE
bool RTREE_QUAL::Batch_Search2(Node* a_node, Rect* a_rect, Region *queryRegions, int queryRegionsSize, int& a_foundCount, vector<int>& hits) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Batch_Search2(a_node->m_branch[index].m_child, a_rect, queryRegions, queryRegionsSize, a_foundCount, hits))
        {
          // The callback indicated to stop searching
          return false;
        }
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        hits.push_back(a_node->m_branch[index].m_data);
      }
    }
...
void initialize_user_metrics(float *norms, int numUsers)
{
  normPerUser = new float[numUsers];
  similarityPerUser = new float[numUsers];

  for (int i = 0; i < numUsers; i++)
  {
    normPerUser[i] = norms[i];
    similarityPerUser[i] = 0.0;
  }
  
}

bool sortbysec(const pair<float,int> &a,
              const pair<float,int> &b)
{
    return (a.first > b.first);
}

vector <pair<float,int>> top_K_Users(int numUsers, int k, float queryNorm)
{
  vector <pair<float, int>> returnedUsers;
  for (int i = 0; i < numUsers; i++)
  {
    float denominator = normPerUser[i] * queryNorm;
    if (denominator == 0.0)
      continue;
    returnedUsers.push_back(make_pair((float)(similarityPerUser[i] / denominator), i));
    similarityPerUser[i] = 0.0;
  }
  ...
  {
    sort(returnedUsers.begin(), returnedUsers.end(),sortbysec);

    vector <pair<float, int>> topKUsers;

    for (int i = 0; i < k; i++)
    {
      topKUsers.push_back(make_pair(returnedUsers[i].first, returnedUsers[i].second));
    }
